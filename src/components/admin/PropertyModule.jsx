 ...
 *    `slug` was originally assumed to be server-managed (backfilled by
 *    2.1) and was intentionally left out of this form as a result — that
 *    assumption was wrong. There is no DB-side trigger or default
 *    generating it for new inserts (2.1's backfill only covered existing
 *    rows at migration time, per its own completion criterion), so new
 *    property creates failed with a NOT NULL constraint violation on
 *    `slug` in production. Fixed post-5.2 by exposing an editable Slug
 *    field with the same client-side `slugify()` + dirty-tracking
 *    pattern `BlogModule.jsx` (5.3) already established for blog posts —
 *    auto-derives from the name until the admin edits it directly, and
 *    `buildPayload` falls back to `slugify(fields.name)` as a final
 *    guard so the column is never sent null.
...
function TagInput({ label, tags, onChange, placeholder, hint, splitPattern }) {
  const [draft, setDraft] = useState("");

  const commit = () => {
    const raw = draft.trim();
    if (!raw) return;
    // If a splitPattern is provided, break a typed/pasted compound value
    // (e.g. "Residential & Commercial") into separate clean tags instead
    // of saving it as one garbled string. Opt-in so unrelated TagInput
    // usages (amenities, key features) that might legitimately contain
    // "&" in a single value are unaffected.
    const parts = splitPattern
      ? raw.split(splitPattern).map((p) => p.trim()).filter(Boolean)
      : [raw];
    const next = [...tags];
    parts.forEach((p) => {
...
function PropertyForm({
  ...
  const [propertyTypes, setPropertyTypes] = useState(() =>
    toArray(property?.property_type),
  );
  ...
  const handleClear = () => {
    if (
      ...
      // remove saved draft when clearing explicitly
      window.localStorage.removeItem(LOCAL_STORAGE_KEY);
    }
    resetForm();
  };
  ...
  const submit = async (e, { asDraft } = {}) => {
    ...
    if (ok) {
      ...
      if (isCreate) {
        // Only reset the create form on successful save (draft or publish)
        resetForm();
      }
    }
  };
        ...
        {activeTab === "basic" && (
          <div className="pm-grid">
            ...
            <Field
              label="Slug"
              htmlFor="f-slug"
              hint="Auto-generated from the property name; edit to override. Used in the public URL."
            >
              <input
                id="f-slug"
                type="text"
            ...
            <Field
              label="Property Type"
              hint="Click any presets that apply below. Each type should be its own entry — don't type combinations like 'Residential & Commercial' into one box."
            >
              <div className="pm-quickpicks">
                {PROPERTY_TYPE_OPTIONS.map((opt) => {
                  const active = propertyTypes.includes(opt);
                  return (
                    <button
                      key={opt}
                      type="button"
                      className={`pm-quickpick${active ? " pm-quickpick--active" : ""}`}
                      aria-pressed={active}
                      onClick={() =>
                        setPropertyTypes(
                          active
                            ? propertyTypes.filter((t) => t !== opt)
                            : [...propertyTypes, opt],
                        )
                      }
                    >
            ...
            <TagInput
              ...
              placeholder="Only a new property type name here, then press Enter"
              hint="For a new/custom type not listed above, type just that one name and press Enter — not the full property name or a combo. If you do type something like 'Residential & Commercial', it will be split into separate types automatically."
              splitPattern={/\s*(?:&|\/|,|;|\band\b)\s*/i}
            />
...
const PROPERTY_LIST_SKELETON_COUNT = 6;

function PropertyListSkeleton() {
  return (
    <div className="pm-list">
      {Array.from({ length: PROPERTY_LIST_SKELETON_COUNT }).map((_, i) => (
        <div className="pm-card" key={i}>
          <Skeleton variant="rect" height={150} className="pm-card__image" />
